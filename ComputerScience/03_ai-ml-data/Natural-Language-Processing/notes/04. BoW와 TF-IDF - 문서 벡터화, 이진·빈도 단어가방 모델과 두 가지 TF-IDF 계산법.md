...
---
> [!NOTE]
> 텍스트 자동 분류 같은 작업을 하려면 문서를 숫자 벡터로 바꿔야 한다 (p.2–3) — **BoW**(등장 여부/횟수) → **TF-IDF**(흔한 단어 감점). 출처 `nlp2026_4장_tfidf.pdf` · 18쪽
...
- 장점: 단순·구현 쉬움 / 단점: 순서·문맥 소실, 차원의 저주, 희소 행렬 → 확장이 TF-IDF·Word2Vec·BERT (p.5)
...
- **방법1** (p.9–12) — TF = 빈도 ÷ 문서 총 단어수 · IDF = $\ln(N / df)$ · 정규화 없음
- **방법2** (p.13–17) — TF = 빈도 그대로 · IDF = $\ln((N+1) / (df+1)) + 1$ · 끝에 L2 정규화
- **L2 정규화** (p.16) — 문서 벡터를 자기 norm $\sqrt{\sum_i v_i^2}$ 으로 나눈다. 문서1은 $\sqrt{1.9163^2 + 1.2231^2 + 1.9163^2} \approx 2.9737$ → 모든 문서가 norm 1
- 비유: 모두가 하는 말은 안 띄고, **특정 학생만 반복한 말**이 그 학생의 키워드 (p.6)
...
전처리는 아직 안 배웠으므로 **조사만 제외**했다고 가정한다 ($N = 4$, 사전 13개 → 13차원).
...
<details>
<summary>11. TF-IDF 값이 0이면 그 단어는 그 문서에 없다는 뜻이다.</summary>

**❌** — 함정. $df = N$ 이면 $\ln(N/N) = 0$ 이라 **있어도 0**이 된다. 과제의 `i`·`learning` 이 그 경우 — 모든 문서에 있어서 변별력이 0인 것이다.

</details>

<details>
<summary>12. L2 정규화를 하면 벡터의 방향까지 바뀜다.</summary>

**❌** — 모든 성분을 같은 수(norm)로 나누므로 **방향은 그대로, 길이만 1**로 맞춰진다. 위젯에서 (3) → (4) 단계로 넘어가면 막대 모양은 그대로고 숫자만 줄어든다.
